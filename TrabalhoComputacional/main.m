...
%% Primeira questão item 4
    ...
    %SINR for the MACRO CELL
    SINR = (powerPico + min(20,360/angle_4)) / (pathLoss(hypot(TM_table(1,1),TM_table(1,2))) + powerNoise);
    aux(1) = capacity(timepercentage_4, SINR);
    
    %SINR for the PICO CELL
    SINR = (powerPico + min(20,360/angle_4)) / (pathLoss(hypot(TM_table(1,1)-PicoTower(1),TM_table(1,2)-PicoTower(2))) + powerNoise);
...
function cdfCompare(datamatrix, extra_param)
    ...
    elseif(extra_param == 4)
        title(strcat('Cenário ', int2str(extra_param), ' macroTime: ', num2str(timepercentage_4) , ' angle: ', num2str(min(20,(360/angle_4)) ) ) )
    else
